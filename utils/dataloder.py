...
class LoadDatasets(Dataset):
    ...
    def _calculate_class_weights(self) -> torch.Tensor:
        ...
        for count in class_counts:
            if count == 0:
                weights.append(0.0)
                logger.warning("One or more classes have 0 samples — check your dataset.")
            else:
...
def create_data_loaders(data_dir: str, 
    ...
    full_dataset = LoadDatasets(data_dir, transform=None, subset='full')
    ...
    train_dataset = LoadDatasets(data_dir, transform=train_transform, subset='train')
    train_dataset.samples = train_samples
    
    val_dataset = LoadDatasets(data_dir, transform=val_transform, subset='val')
    val_dataset.samples = val_samples
    
    test_dataset = LoadDatasets(data_dir, transform=test_transform, subset='test')
...
def visualize_batch(data_loader: DataLoader, num_samples: int):
